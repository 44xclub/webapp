'use client'

import { useState, useCallback, useMemo } from 'react'
import { type UseFormReturn } from 'react-hook-form'
import { Input, Textarea, Select } from '@/components/ui'
import { repeatPatternLabels, weekdayLabels } from '@/lib/utils'
import { Plus, Trash2, Check } from 'lucide-react'
import type { PersonalFormData } from '@/lib/schemas'
import type { PersonalTask } from '@/lib/types'

interface PersonalFormProps {
  form: UseFormReturn<PersonalFormData>
  blockId?: string
  onTaskToggle?: (blockId: string, tasks: PersonalTask[]) => Promise<unknown>
}

function generateId(): string {
  return Math.random().toString(36).substring(2, 8)
}

export function PersonalForm({ form, blockId, onTaskToggle }: PersonalFormProps) {
  const {
    register,
    watch,
    setValue,
    getValues,
    formState: { errors },
  } = form

  const repeatPattern = watch('repeat_rule.pattern') || 'none'
  const selectedWeekdays = watch('repeat_rule.weekdays') || []
  const watchedPayload = watch('payload') as { tasks?: PersonalTask[] } | undefined
  const payloadTasks = watchedPayload?.tasks

  const [newTaskText, setNewTaskText] = useState('')
  const [showAddInput, setShowAddInput] = useState(false)
  const [toastMessage, setToastMessage] = useState<string | null>(null)

  const tasks = useMemo(() => {
    const t = payloadTasks || []
    return [...t].sort((a, b) => a.sort_order - b.sort_order)
  }, [payloadTasks])

  const showToast = useCallback((msg: string) => {
    setToastMessage(msg)
    setTimeout(() => setToastMessage(null), 2000)
  }, [])

  const updateTasks = useCallback(async (newTasks: PersonalTask[]) => {
    // Set the full payload object to avoid nested path issues
    const currentPayload = (getValues('payload') as Record<string, unknown>) || {}
    setValue('payload', { ...currentPayload, tasks: newTasks } as any, { shouldDirty: true })

    // Autosave for existing blocks
    if (blockId && onTaskToggle) {
      try {
        await onTaskToggle(blockId, newTasks)
      } catch {
        showToast("Couldn't save")
      }
    }
  }, [blockId, onTaskToggle, setValue, getValues, showToast])

  const handleToggleTask = useCallback(async (taskId: string) => {
    const rawPayload = getValues('payload') as { tasks?: PersonalTask[] } | undefined
    const currentTasks = rawPayload?.tasks || []
    const newTasks = currentTasks.map(t => {
      if (t.id !== taskId) return t
      const newDone = !t.done
      return {
        ...t,
        done: newDone,
        completed_at: newDone ? new Date().toISOString() : null,
      }
    })
    await updateTasks(newTasks)
  }, [getValues, updateTasks])

  const handleAddTask = useCallback(async () => {
    const trimmed = newTaskText.trim()
    if (!trimmed) return
    if (trimmed.length > 120) return

    const rawPayload = getValues('payload') as { tasks?: PersonalTask[] } | undefined
    const currentTasks = rawPayload?.tasks || []
    if (currentTasks.length >= 30) {
      showToast('Maximum 30 tasks')
      return
    }

    const maxOrder = currentTasks.length > 0
      ? Math.max(...currentTasks.map(t => t.sort_order))
      : -1

    const newTask: PersonalTask = {
      id: generateId(),
      text: trimmed,
      done: false,
      sort_order: maxOrder + 1,
      created_at: new Date().toISOString(),
      completed_at: null,
    }

    const newTasks = [...currentTasks, newTask]
    setNewTaskText('')
    await updateTasks(newTasks)
  }, [newTaskText, getValues, updateTasks, showToast])

  const handleDeleteTask = useCallback(async (taskId: string) => {
    const rawPayload = getValues('payload') as { tasks?: PersonalTask[] } | undefined
    const currentTasks = rawPayload?.tasks || []
    const newTasks = currentTasks.filter(t => t.id !== taskId)
    await updateTasks(newTasks)
  }, [getValues, updateTasks])

  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      e.preventDefault()
      handleAddTask()
    }
  }, [handleAddTask])

  const toggleWeekday = (day: number) => {
    const current = selectedWeekdays || []
    const updated = current.includes(day)
      ? current.filter((d) => d !== day)
      : [...current, day]
    setValue('repeat_rule.weekdays', updated)
  }

  return (
    <div className="space-y-4">
      {/* Title */}
      <Input
        label="Title"
        placeholder="e.g., Doctor appointment, Meeting"
        {...register('title')}
        error={errors.title?.message}
      />

      {/* Repeat Pattern */}
      <Select
        label="Repeat"
        options={Object.entries(repeatPatternLabels).map(([value, label]) => ({
          value,
          label,
        }))}
        {...register('repeat_rule.pattern')}
      />

      {/* Weekday selection for weekly */}
      {repeatPattern === 'weekly' && (
        <div>
          <label className="block text-[13px] font-medium text-[rgba(238,242,255,0.72)] mb-2">
            Select Days
          </label>
          <div className="flex gap-2 flex-wrap">
            {weekdayLabels.map((label, index) => (
              <button
                key={label}
                type="button"
                onClick={() => toggleWeekday(index)}
                className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${
                  selectedWeekdays.includes(index)
                    ? 'bg-[rgba(255,255,255,0.09)] text-[#eef2ff] border border-[rgba(255,255,255,0.14)]'
                    : 'text-[rgba(238,242,255,0.45)] border border-[rgba(255,255,255,0.06)] hover:border-[rgba(255,255,255,0.14)]'
                }`}
              >
                {label}
              </button>
            ))}
          </div>
        </div>
      )}

      {/* Custom interval */}
      {repeatPattern === 'custom' && (
        <Input
          type="number"
          label="Every N days"
          placeholder="e.g., 2 for every other day"
          min={1}
          {...register('repeat_rule.interval', {
            valueAsNumber: true,
          })}
        />
      )}

      {/* Tasks Section */}
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-[13px] font-medium text-[rgba(238,242,255,0.72)]">
            Tasks
          </label>
          <button
            type="button"
            onClick={() => setShowAddInput(true)}
            className="flex items-center gap-1 text-[12px] text-[var(--accent-primary)] font-medium hover:text-[#60a5fa] transition-colors"
          >
            <Plus className="h-3.5 w-3.5" />
            Add
          </button>
        </div>

        {/* Task rows */}
        {tasks.length > 0 && (
          <div className="space-y-0.5 mb-2">
            {tasks.map((task) => (
              <div
                key={task.id}
                className="flex items-center gap-2.5 py-2 px-2 rounded-[8px] hover:bg-[rgba(255,255,255,0.02)] group"
              >
                {/* Checkbox */}
                <button
                  type="button"
                  onClick={() => handleToggleTask(task.id)}
                  className="flex-shrink-0 flex items-center justify-center h-[44px] w-[44px] -m-2"
                >
                  <div
                    className={`h-[18px] w-[18px] rounded-[5px] border transition-colors flex items-center justify-center ${
                      task.done
                        ? 'bg-[var(--accent-primary)] border-[var(--accent-primary)]'
                        : 'border-[rgba(255,255,255,0.2)] bg-transparent'
                    }`}
                  >
                    {task.done && <Check className="h-3 w-3 text-white" />}
                  </div>
                </button>

                {/* Text */}
                <span
                  className={`flex-1 text-[13px] truncate ${
                    task.done
                      ? 'text-[rgba(238,242,255,0.35)] line-through decoration-[rgba(238,242,255,0.2)]'
                      : 'text-[rgba(238,242,255,0.85)]'
                  }`}
                >
                  {task.text}
                </span>

                {/* Delete */}
                <button
                  type="button"
                  onClick={() => handleDeleteTask(task.id)}
                  className="flex-shrink-0 p-1.5 text-[rgba(238,242,255,0.25)] hover:text-[rgba(239,68,68,0.8)] active:text-[rgba(239,68,68,0.8)] transition-colors"
                >
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              </div>
            ))}
          </div>
        )}

        {/* Add task input */}
        {showAddInput && (
          <div className="flex items-center gap-2">
            <input
              type="text"
              value={newTaskText}
              onChange={(e) => setNewTaskText(e.target.value)}
              onKeyDown={handleKeyDown}
              onBlur={() => {
                if (!newTaskText.trim()) setShowAddInput(false)
              }}
              placeholder="New task..."
              maxLength={120}
              autoFocus
              className="flex-1 h-[38px] px-3 text-[13px] bg-[var(--surface-2)] text-[#eef2ff] border border-[rgba(255,255,255,0.10)] rounded-[8px] placeholder:text-[rgba(238,242,255,0.3)] focus:outline-none focus:border-[rgba(59,130,246,0.5)]"
            />
            <button
              type="button"
              onClick={handleAddTask}
              disabled={!newTaskText.trim()}
              className="h-[38px] px-3 text-[12px] font-medium text-[var(--accent-primary)] hover:text-[#60a5fa] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              Add
            </button>
          </div>
        )}

        {/* Toast */}
        {toastMessage && (
          <div className="mt-2 text-[11px] text-[rgba(239,68,68,0.8)] font-medium">
            {toastMessage}
          </div>
        )}
      </div>

      {/* Notes */}
      <Textarea
        label="Notes"
        placeholder="Optional notes..."
        {...register('notes')}
      />
    </div>
  )
}
